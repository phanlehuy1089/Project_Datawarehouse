...
public class LoadDataToWarehouse {
	public static void loadDataToWarehouse(InfoConfig infoConfig, String srcDatabase, String tarDatabase, String srcTable, String tarTable, String fields, String exclusiveField) {
		...
		String sql = "INSERT INTO " + tarDatabase + "." + tarTable + " (" + fields + ",date_last_change) SELECT " + fields + ",current_timestamp() FROM " + srcDatabase + "." + srcTable + " WHERE " + exclusiveField + " <> '0' GROUP BY " + exclusiveField + " ORDER BY " + exclusiveField;
		try {
			PreparedStatement ps = connection.prepareStatement(sql);
			ps.executeUpdate();
			connection.close();
			System.out.println("Load Data Into Warehouse Successfully!\n");
